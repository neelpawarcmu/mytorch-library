    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for l, layer in enumerate(self.rnn):
                # Run the rnn cell with the correct parameters and update. Update hidden
                x_t = hidden[l,:,:] = layer.forward(x_t, self.hiddens[-1][l])
            # Similar to above, append a copy of the current hidden array to the hiddens list
            self.hiddens.append(hidden.copy())
        # Get the outputs from the last time step using the linear layer and return it
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in reversed(range(seq_len)):
            for l in reversed(range(self.num_layers)):
                h_prev_l = self.hiddens[t+1][l-1] if l > 0 else self.x[:,t,:]
                h_prev_t = self.hiddens[t][l]
                h = self.hiddens[t+1][l]
                dx_tl, dh_tl = self.rnn[l].backward(dh[l], h, h_prev_l, h_prev_t) #pass dh[l] as delta
                dh[l] = dh_tl
                dh[l-1] += dx_tl if l>0 else 0
